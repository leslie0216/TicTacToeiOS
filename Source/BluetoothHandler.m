...
{
    PeripheralInfo *info =  self.discoveredPeripherals[UUID];
    CCLOG(@"Connecting to peripheral name = %@ id = %@", info.name, UUID);
...
- (void) centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary<NSString *,id> *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    if (self.discoveredPeripherals[peripheral.identifier.UUIDString] == nil) {
        ...
        NSDictionary *userInfo = @{ @"peerName": name,
                                    @"peerUUID": peripheral.identifier.UUIDString};
...
{
    NSString *msg = [NSString stringWithFormat:@"Disconnect from %@", self.discoveredPeripherals[peripheral.identifier.UUIDString].name];
    CCLOG(@"%@", msg);
...
#if __CC_PLATFORM_IOS
        NSString *name = localName;//[UIDevice currentDevice].name;
#elif __CC_PLATFORM_MAC
        NSString *name = localName;//[[NSHost currentHost] localizedName];
#endif        
